...
" }}}
" Plugin: Atone - Undo trees via :Atone {{{
Plug 'XXiaoA/atone.nvim', {'done': 'call s:ConfigAtone()'}

function s:ConfigAtone()
lua <<EOF
	require('atone').setup({
	})
EOF
...
" }}}
" Plugin: Tiny-Glimmer - Yank replacement with animations {{{
Plug 'rachartier/tiny-glimmer.nvim', {'done': 'call s:ConfigTinyGlimmer()'}
" @url https://github.com/rachartier/tiny-glimmer.nvim

function s:ConfigTinyGlimmer()
lua <<EOF
